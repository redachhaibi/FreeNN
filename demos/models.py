...
class DataMaster:
    ...
    fct = _get_attr(self, "load_"+ dataset_name, "There's no dataset by the name: {attr_name}")
    fct()
    ...
    def load_cifar10():
        ...
        self.trainloader = torch.utils.data.DataLoader(trainset, batch_size=self.mininatch_size, shuffle=True, num_workers=self.num_workers),
        self.testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform),
        self.testloader = torch.utils.data.DataLoader(testset, batch_size=self.mininatch_size, shuffle=False, num_workers=self.num_workers),
        self.classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck'),
        self.in_size =  32*32*3,
        self.out_size =  10

    def load_mnist(mininatch_size, num_workers=8):
        transform = transforms.Compose(
            [
                transforms.ToTensor(),
            ]
...
class BatchReporter:
    ...
    def save_report(self, model):
        ...
        for key, value in fct():
            path = os.path.join( self.dir, key)
            if not os.path.exists(path):
                os.mkdir(path)

            filename = os.path.join( path, "%s.npy" % self.tick ) 
            value = value.cpu().numpy()
            numpy.save(filename, value)

class Trainer(object):
    """docstring for Trainer"""
    def __init__(self, loss_name, optimizer_kwargs):
        ...
        self.optimizer = optimizer_fct(optimizer_kwargs)

    def _one_pass(net, data, reporter, training):
    ...
    def train(self, np_epochs, net, data_master, reporter):
        from tqdm import trange

        pbar = trange(nb_epochs)
        for epoch in pbar:
            train_loss = 0.0
            for data in data_master.trainloader:
                self._one_pass(data, reporter=reporter, training=True)
            
            for data in data_master.testloader:
                self._one_pass(data, reporter=None, training=False)
